...
用 Playwright 驱动本机 Chrome，辅助处理雨课堂作业、课程学习内容和进度核验。作业模式会读取题目 → 调用视觉大模型作答 → 自动勾选选项，**可选自动提交并翻页**；刷课模式会进入学习内容，按进度处理视频/图文单元。
...
3. 选 **① 单份作业**、**② 批量做整门课**，或 **③ 自动刷课**；
4. 作业模式选择：**试运行 / 只勾选不提交 / 勾选并自动提交**；刷课模式按提示填写倍速、静音和单元上限。
